...
public class GameManager : MonoBehaviour
{
    ...
    {
        // TODO: Debug.Log($"{entities[entityNum].name} starts its turn!");

        if (entities[entityNum].GetComponent<PlayerScript>())
            isPlayerTurn = true;
        else if (entities[entityNum].IsSentient)
            Action.SkipAction(entities[entityNum]); // TODO: No AI yet, so skip the turn
    }
